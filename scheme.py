...
def do_lambda_form(vals, env):
    ...
    if vals.second.second is not nil:
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        function = do_lambda_form(Pair(target.second,vals.second), env)
        env.define(target[0], function)
        return target[0]
    else:
...
def check_formals(formals):
    ...
    for i in range(len(formals)):
        sym = formals[i]
        if scheme_symbolp(sym) and (sym not in check):
            check += [sym]
        else:
